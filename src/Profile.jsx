...
export default function Profile() {
  ...
  // Photo upload state & ref to fix the "second upload" bug
  ...
  useEffect(() => {
    ...
    backgroundRefresh();
  }, [userId, navigate, backgroundRefresh]);

  const calculateEligibility = () => {
    if (!user?.lastDonationDate) return { eligible: true, days: 0 };
    const diffDays = Math.ceil(
      Math.abs(new Date() - new Date(user.lastDonationDate)) /
        (1000 * 60 * 60 * 24),
    );
  ...
  const handleEditSave = async () => {
    ...
    try {
      const body = {
        ...
        hospitalOrOrg:
          user.role === "REQUESTER" ? editForm.hospitalOrOrg || null : null,
        contactNumber:
          user.role === "REQUESTER" ? editForm.contactNumber || null : null,
      };
  ...
  const handlePhotoUpload = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        // Physically clear the input so you can upload another photo immediately
  ...
  const handleNavClick = (item) => {
    if (item === "Overview")
      navigate("/dashboard", { state: { user: passedUser } });
  };
        ...
        >
          ...
          >
            {user.role === "DONOR" && (
              ...
              >
                <div>
                  ...
                  >
                    {eligible
                      ? "You are cleared to commit to donation requests."
            ...
            >
              ...
              {user.role === "DONOR" &&
                readRow("Total Donations", String(user.totalDonations ?? 0))}
              {user.role === "DONOR" &&
                readRow(
                  "Last Donation",
                  user.lastDonationDate
              ...
              {user.role === "REQUESTER" &&
                readRow("Last Donation", undefined, false, true)}
            </div>
            ...
            >
              ...
              >
                ...
                <button
                  ...
                  style={{
                    backgroundColor: isEditing ? "#f1f5f9" : "#fee2e2",
                    color: isEditing ? "#64748b" : "#D32F2F",
                    padding: "8px 20px",
                    borderRadius: "8px",
                    border: "none",
                    fontWeight: "800",
              ...
              {isEditing ? (
                ...
                <>
                  ...
                  {user.role === "DONOR" &&
                    readRow("Blood Type", user.bloodType, true, false)}
                  {user.role === "REQUESTER" &&
                    readRow("Hospital / Org", user.hospitalOrOrg, false, false)}
                  {user.role === "REQUESTER" &&
                    readRow("Contact Number", user.contactNumber, false, true)}
                  {user.role === "DONOR" &&
                    readRow("Blood Type", undefined, false, true)}
                </>
              )}
            </div>
